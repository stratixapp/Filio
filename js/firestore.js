...
const FS = (() => {
  ...
  async function addClient(firmId, data) {
    ...
    // Auto-generate compliance deadlines for the new client
    try {
      if (typeof ComplianceRadar !== 'undefined') {
        const newClient = { id: clientRef.id, ...data };
        ComplianceRadar.generateDeadlinesForClient(firmId, newClient).catch(() => {});
      }
